...
from test_project.db.config import TORTOISE_ORM
from test_project.users import utils
from test_project.users.schemas import UserCreate
...
def work(mailserver: bool = typer.Option(False)):
    ...
    manager.add_process("server", f"aerich upgrade && {sys.executable} -m test_project dev", env=project_env)
    manager.add_process("worker", f"{sys.executable} -m test_project run-worker", env=project_env)
    if mailserver:
        manager.add_process("mailserver", f"{sys.executable} -m test_project run-mailserver")
...
def run_prod_server():
    ...
    class APPServer(Application):
        ...
        def load(self):
            return util.import_app("test_project.main:app")
...
def start_app(app_name: str):
    ...
    files = {
        "__init__.py": "",
        "models.py": "from test_project.db.models import TimeStampedModel",
...
def run_worker(reload: bool = typer.Option(True)):
    ...
    if reload:
        subprocess.run(["hupper", "-m", "saq", "test_project.worker.settings", "--web"])
    else:
        subprocess.run(["python", "-m", "saq", "test_project.worker.settings", "--web"])
